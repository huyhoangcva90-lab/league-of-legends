...
Add-Contract 'Team' 'My Team marker defaults to Blue and owns the lane metric read' ((Has-All $app @('function\s+myTeamMarker','data-my-team-side','function\s+myTeamSide','dataset\.perspective=myTeamSide\(\)','team-sheet-versus','teamLaneMetric','function\s+teamLaneMetricRead','function\s+teamLaneMetricPicker','data-team-lane-metric')) -and (Has-All $style @('\.team-lane-read','\.team-lane-metric-picker')) -and $app -notmatch 'team-sheet-score') 'Exclusive Blue/Red checkbox; neutral resolves to Blue; VS selects Win rate, Win lane, Gold edge or Teamfight and displays only My Team value'
...
Add-Contract 'Team' 'Composition strategy implements Team row 41, Setting ranks and fight-aware phase scoring' ((Has-All $app @('teamMatrixTab','teamStrategyPhase','sheetTacticalCounts','sheetCombatPrimary','sheetCombatSecondary','sheetMacroType','sheetWinConditionSignature','sheetDetailedRuleResults','sheetRankedTeamTypes','teamTypePrimary','teamTypeSecondary','Setting!O31 / O32','Setting!P31 / P32','teamStrategyMetricScores=function','sheetPhaseBaseScore','teamfightPhaseQuality','teamStrategyPhaseDetail','teamStrategyPentagon','strategyPhaseCompareRow','Team!E41:K41','Team!S41:Y41','data-team-matrix-tab','team-strategy-overview')) -and (Has-All $style @('\.team-matrix-tabs','\.team-strategy-overview','\.team-strategy-scoreboard','\.team-strategy-sides','\.team-strategy-signature','\.team-strategy-classification','\.strategy-primary','\.team-strategy-pentagon','\.team-pentagon-fill','\.phase-detail')) -and $teamRulesText -match 'sheetPhaseRoleScores' -and $teamRulesText -match 'sheetTeamfightPhaseWeights' -and $teamRulesText -match 'sheetMacroPhaseWeights' -and $teamRulesText -match 'sheetWinConditionLabels') 'AN:AR pentagon, fixed three-phase comparison without a visible phase selector, teamfight/map phase adjustments, Team combat IFS, Split+2 macro, and Setting five-category ranking'
