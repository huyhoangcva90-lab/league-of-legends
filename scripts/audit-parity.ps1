...
Add-Contract 'Team' 'Champion picker preserves the complete champion decision model' ((Has-All $app @('pickerFilters:\{role:\[\],className:\[\],range:\[\],damage:\[\],damageProfile:\[\],attackStyle:\[\],combat:\[\],macro:\[\],position:\[\],profiles:\[\],tier:\[\],difficulty:\[\],early:\[\],mid:\[\],late:\[\],capabilities:\[\]\}','PICKER_SINGLE_FILTERS','data-picker-filter-select','function\s+pickerChampionSignals')) -and (Has-All $style @('\.compact-icons','\.picker-card-signals','\.picker-icon-signal::after'))) 'Role, class OR, range, damage, source, combat OR, macro, formation OR, profiles OR, tier, difficulty, three phases and skill effects'
Add-Contract 'Team' 'Champion mirror exposes strengths, weaknesses and counter guidance' ((Has-All $app @('function\s+counterGuideFor','LANING AGAINST','STRATEGY VS','team-guide-source')) -and (Has-All $style @('\.team-guide-copy \.strength','\.team-guide-copy \.weakness','\.team-guide-copy \.laning','\.team-guide-copy \.strategy'))) 'Imported guide strengths/weaknesses plus derived Laning Against and Strategy VS blocks'
