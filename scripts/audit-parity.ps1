...
Add-Contract 'Team' 'Champion guidance separates mirrored strengths from enemy-only matchup advice' ((Has-All $app @('teamGuideTab','data-team-guide-tab','function\s+teamCounterCard',"myTeamSide\(\)==='blue'\?'red':'blue'",'LANING AGAINST','STRATEGY VS','team-guide-source')) -and (Has-All $style @('\.team-guide-tabs','\.team-counter-list','\.enemy-only','\.team-guide-copy \.strength','\.team-guide-copy \.weakness','\.team-guide-copy \.laning','\.team-guide-copy \.strategy'))) 'Strength/weakness mirrors both teams; the matchup tab renders only the enemy side selected by My Team'
