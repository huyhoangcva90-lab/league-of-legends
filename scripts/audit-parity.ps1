...
Add-Contract 'Teamfight' 'Wiki subclasses and derived execution archetypes' ((Has-All $app @('TEAMFIGHT_ARCHETYPES','TEAMFIGHT_DERIVED_ROLES','function\s+advancedTeamfightPanel','function\s+teamfightExecutionSide','profiles\?\.hyperCarry','ratings\?\.toughness')) -and (Test-Path (Join-Path $root 'assets\teamfight-archetypes\manifest.json')) -and @(Get-ChildItem (Join-Path $root 'assets\teamfight-archetypes') -Filter '*.png').Count -ge 16) '13 Wiki subclasses + Hard Carry, Poke Controller and Finisher rules with dedicated icons'
