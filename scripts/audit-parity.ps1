...
Add-Contract 'Teamfight' 'Custom Combat Class and execution archetypes' ((Has-All $app @('COMBAT_CLASS_DATA','function\s+combatClassMatches','function\s+championCombatClasses','function\s+advancedTeamfightPanel','function\s+teamfightExecutionSide','profiles\?\.hyperCarry','ratings\?\.toughness')) -and (Test-Path (Join-Path $root 'src\data\combat-classes.json')) -and (Test-Path (Join-Path $root 'assets\teamfight-archetypes\manifest.json')) -and @(Get-ChildItem (Join-Path $root 'assets\teamfight-archetypes') -Filter '*.png').Count -ge 15) '15 custom Combat Classes independent from Riot primary/secondary class, including distinct Vanguard, Warden, Hard Carry and two Catchers'
