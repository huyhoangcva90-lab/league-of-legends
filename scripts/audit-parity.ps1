...
Add-Contract 'Team' 'Champion guidance separates mirrored strengths from enemy-only matchup advice' ((Has-All $app @('teamGuideTab','data-team-guide-tab','bindStateTabs',"myTeamSide\(\)==='blue'\?'red':'blue'",'function\s+teamCounterCard','LANING AGAINST','STRATEGY VS','team-guide-source')) -and (Has-All $style @('\.team-guide-tabs','\.team-counter-list','\.enemy-only','\.team-guide-copy \.strength','\.team-guide-copy \.weakness','\.team-guide-copy \.laning','\.team-guide-copy \.strategy')) -and $index -match 'team-strategy-v2') 'Direct rendered-tab binding plus cache-busted assets; matchup tab renders only the enemy selected by My Team'
Add-Contract 'Team' 'Composition strategy implements the Team row 41 formula band' ((Has-All $app @('teamMatrixTab','teamStrategyPhase','function\s+teamStrategyMetricScores','function\s+teamStrategyPhaseScore','function\s+teamCombatShape','function\s+teamStrategySignature','function\s+teamStrategyDeployment','Team!E41:K41','Team!S41:Y41','data-team-matrix-tab','data-team-strategy-phase')) -and (Has-All $style @('\.team-matrix-tabs','\.team-strategy-scoreboard','\.team-strategy-sides','\.team-strategy-signature')) -and $teamRulesText -match 'strategyRoleWeights' -and $teamRulesText -match 'strategyPhaseMetricWeights') 'Damage/Toughness/Control/Mobility/Utility use role and phase weights; combat form and deployment are derived for both sides'
